...
# Perform PCA
test_components = 2 * n_qubits
pca = PCA(n_components=test_components)
x_train = pca.fit_transform(X_train)
x_test = pca.transform(X_test)

# Choose only a subset of data
...
for r in range(reps):
    ...
    if n_qubits == 1:
        limit_low, limit_up, step = 1, 10, 1
    elif n_qubits == 2:
        limit_low, limit_up, step = 3, 14, 1
    elif n_qubits == 3:
        limit_low, limit_up, step = 4, 19, 1
    elif n_qubits == 4:
        limit_low, limit_up, step = 6, 23, 1
    elif n_qubits == 5:
        limit_low, limit_up, step = 8, 27, 1
